...
- **文件操作**：读写搜索补丁，安全沙箱保护
- **终端**：命令执行 + 后台进程管理
- **Web**：搜索、内容提取、浏览器自动化（Chromium + 截图视觉请求 + Playwright 缓存/headless-shell 发现 + 原始 CDP 派发 + 云浏览器 Provider 就绪状态 + Provider CDP endpoint 路由）
...
- CLI Skin Engine — `hakimi skin list|inspect|set|path` 与网关 `/skin` 可发现内置和 `~/.hakimi/skins/*.yaml` 主题，缺失字段继承 `default`，持久化 `display.skin`，并把选中的 branding/colors/logo/hero 应用到 CLI 启动横幅
...
- [x] CLI Skin Engine，支持内置/用户 YAML 主题、`display.skin` 持久化和启动横幅主题化
