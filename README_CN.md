...
- CLI Skin Engine — `hakimi skin list|inspect|set|path` 与网关 `/skin` 可发现内置和 `~/.hakimi/skins/*.yaml` 主题，缺失字段继承 `default`，持久化 `display.skin`，并把选中的 branding/colors/logo/hero 应用到 CLI 启动横幅，同时驱动 TUI 思考态 spinner faces/verbs/wings
...
- [x] CLI Skin Engine，支持内置/用户 YAML 主题、`display.skin` 持久化、启动横幅主题化和 TUI spinner 主题化
