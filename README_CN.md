...
- **v0.3.142 Kanban board 隔离**
  ...
  - Board 管理面：新增 `/kanban boards list|show|create|switch`，通过 `~/.hakimi/kanban/current` 维护当前 board。
  - 安全 slug 边界：board slug 会拒绝路径穿越和不安全名称，同时保留 legacy `~/.hakimi/kanban.db` 默认 board 路径。
...
- **v0.3.139 紧凑 read_file 行号**
  ...
  - 更低 token 成本：Rust 原生文件读取路径统一使用 Hermes 当前标准格式，避免无意义 gutter 空格进入上下文。
  - 新增离线回归覆盖 offset/limit 读取，并确认不会输出 padded gutter。
...
- **v0.3.135 Web/媒体工具 URL 安全**
  ...
  - 工具管理的 HTTP client 会在重定向目标越过公网 URL 边界时停止跟随，避免被 3xx 跳转带入内网。
  - 可信本地部署可用 `HAKIMI_ALLOW_PRIVATE_URLS=true` 或 `HERMES_ALLOW_PRIVATE_URLS=true` 开启 private 目标，但元数据端点仍始终阻断。
...
- **v0.3.133 Write Safe Root Sandbox**
  ...
  - `/etc`、`/private/etc`、`/root/.ssh`、`/proc`、`/sys` 和 `/dev` 等敏感系统/凭据路径仍会被静态拒绝，即使 safe root 配得很宽也不会放行。
  - 文件变更工具统一使用 `Path::is_absolute()` 判断绝对路径，避免 Windows `C:\...` 路径被误拼到当前工作目录下。
...
- **v0.3.128 Skills bundled sync/update**
  ...
  - 未修改的已同步技能会随 bundled 来源更新；用户修改过的技能会保留，用户删除过的技能不会被静默重新加入。
  - gateway `/skills sync --source <dir>` 复用同一摘要/JSON 响应路径，不回显 skill 正文。
...
- **v0.3.126 Skills template preprocessing**
  ...
  - 可信 skill 包可通过 loader options 或 `HAKIMI_SKILLS_INLINE_SHELL` 显式启用 `` !`cmd` `` 展开；命令在 skill 目录执行，并带超时和输出长度上限。
  - preprocessing 只在安全扫描、平台门控和 provenance 合并之后运行，避免未信任的 SKILL.md 文本直接进入运行时 prompt。

- **v0.3.125 Skills platform-gated loading**
  - 对齐 Hermes skill metadata：`SKILL.md` frontmatter 现在可以用 scalar 或 list 声明 `platforms`。
  - Hakimi 会在 skill 进入运行时 prompt 前识别 `macos`/`darwin`、`windows`/`win32`、`linux`、`termux` 与 `android` 别名。
  - 不兼容当前 OS 的技能会被跳过，避免把平台专属操作说明注入到无关会话。

- **v0.3.124 Skills Hub manifest install policy**
  - 对齐 Hermes Skills Hub 的一个离线可验证切片：顶层 `hakimi skills browse|search|inspect|install|list|path` 可读取本地 `.hub/index.json` manifest，并与 gateway `/skills browse|search|inspect|install` 共用同一响应逻辑。
  - community 技能必须显式传入 `--trust-community` 才允许非交互安装；builtin/trusted 技能通过安全扫描后可直接安装。
  - 安装流程会校验 bundle 路径、扫描 `SKILL.md`、阻断 traversal/symlink 替换，写入 `.hub/lock.json` provenance 并追加 `.hub/audit.log`，不回显技能正文。
...
- **v0.3.122 Skills provenance metadata**
  ...
  - 支持 Hermes 风格 `metadata.hermes` provenance 字段和显式顶层 `provenance` block，并在展示前归一化空值与控制字符。
  - skill 摘要和 gateway `/skills` 会显示紧凑的 `source/trust` 标签，便于区分 local、official、trusted 与 community 技能。
...
- **文件**: read_file (紧凑 `N|content` 行号), write_file, search_files, patch
