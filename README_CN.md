...
- **文件操作**：读写搜索补丁，安全沙箱保护
- **终端**：命令执行 + 后台进程管理
- **Web**：搜索、内容提取、浏览器自动化（Chromium + 截图视觉请求 + Playwright 缓存/headless-shell 发现 + CDP endpoint 状态/探针）
...
- 实时流式输出，支持 flood-control 退避、按平台配置 preview 策略，长回复会按平台限制做 UTF-8 安全分片
