...
- HTTP 插件系统，YAML 模板
- HTTP API 发现端点 — OpenAI 兼容 `/v1/models`、`/v1/capabilities`、`/v1/skills`、`/v1/toolsets`、非流式文本 `/v1/chat/completions`、支持 `previous_response_id` 链式续写的非流式 `/v1/responses`、带 SSE 生命周期事件快照的可轮询且可取消 `/v1/runs`，以及会话消息/搜索能力发现，方便外部 UI 探测能力
