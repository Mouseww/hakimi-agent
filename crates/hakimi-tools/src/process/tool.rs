        ...
        match action {
            "start" => {
                let command = args.get("command").and_then(|v| v.as_str())
                    .ok_or_else(|| HakimiError::ToolSimple("command required".into()))?;
            ...
            "poll" => {
                let sid = args.get("session_id").and_then(|v| v.as_str())
                    .ok_or_else(|| HakimiError::ToolSimple("session_id required".into()))?;
            ...
            "write" | "submit" => {
                ...
                let data = args.get("data").and_then(|v| v.as_str())
                    .ok_or_else(|| HakimiError::ToolSimple("data required".into()))?;
