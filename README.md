...
- **Productivity**: todo, Kanban boards with profile routing, worker logs, event trails, diagnostics, notification subscriptions, swarm graph creation, dashboard read/write management, cron scheduler with interval and five-field cron expressions
...
- Cron jobs scheduled from chat with `/cron add`, including `30m` / `2h` intervals and five-field cron syntax such as `*/15 * * * *` or `0 9 * * MON-FRI`
