...
- Telegram · Discord · Slack · Mattermost · Webhook · Signal · SMS/Twilio · WhatsApp Business Cloud · Home Assistant · Matrix · DingTalk · WeCom · Feishu/Lark · WeChat (via iLink/ClawBot)
...
- HTTP API discovery — OpenAI-compatible `/v1/models`, `/v1/capabilities`, and non-streaming text `/v1/chat/completions` for external UI feature detection
...
├── hakimi-gateway/       # 14 runtime-exposed platform adapters
...
- [x] 61+ built-in tools
- [x] 14 runtime-exposed platform adapters
- [x] Gateway target directory + send_message channel resolution
- [x] MCP client + CLI/gateway server catalog
- [x] HTTP API model/capability discovery + text Chat Completions
