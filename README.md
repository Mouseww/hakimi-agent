...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.3.259-blue?style=for-the-badge" alt="Version">
...
WebUI Settings can update the runtime password without restarting: API and OpenAI-compatible routes always pass through the dynamic Bearer-token middleware, while an empty password keeps local development open. The WebUI Control Center can now create, pause, resume, run-now, and delete persisted cron jobs via `/api/cron/jobs`, and the mobile layout lets the conversation title toggle the session list so phones keep the chat area usable.
