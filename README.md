...
### 🌟 What's New
- **v0.3.143 Credential pool reset-window sync**:
  - **Hermes Pool Recovery Parity**: provider failures can now preserve explicit reset timestamps and retry-after windows instead of falling back to a fixed cooldown.
  - **Safe Diagnostics**: credential pool stats retain redacted, capped provider error messages alongside status code, reason, and retry window metadata.
  - **Re-auth Recovery**: fresh singleton OAuth credentials can replace stale dead entries by source, clearing terminal state without duplicating the old credential.
...
- **v0.3.141 TUI slash command autocomplete**:
  ...
  - **Non-Disruptive Tab Behavior**: Tab still toggles the tools panel for normal chat input; it switches to completion only while the cursor is in the first slash-command token and shows bounded candidate hints for ambiguous prefixes.
- **v0.3.140 Credential pool dead-entry cleanup**:
  - **Hermes Pool Hygiene Parity**: dead manual credentials now carry status timestamps and are pruned after a 24-hour quiet window.
  - **OAuth Singleton Safety**: singleton-seeded OAuth credentials such as `device_code` stay quarantined while dead, preventing stale singleton tokens from silently re-entering rotation.
  - **Operator Visibility**: pool stats now expose the last status timestamp alongside status code and reason for safer diagnostics.
...
- **v0.3.135 URL safety for web/media tools**:
  ...
  - **Trusted Local Override**: `HAKIMI_ALLOW_PRIVATE_URLS=true` or `HERMES_ALLOW_PRIVATE_URLS=true` can enable private targets for local deployments, while metadata endpoints remain blocked.
...
- **v0.3.128 Skills bundled sync/update**:
  ...
  - **Gateway Visibility**: gateway `/skills sync --source <dir>` uses the same summary/JSON response path without exposing skill bodies.
...
20+ error types auto-classified: auth failure -> rotate key; terminal OAuth failure -> quarantine credential; stale dead manual credentials -> prune after 24 hours; rate limit -> exponential backoff; context overflow -> trigger compression; model not found -> fallback model.
