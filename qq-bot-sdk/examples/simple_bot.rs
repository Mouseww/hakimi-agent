...
impl EventHandler for MyBot {
    async fn on_ready(&self, ready: &ReadyEvent) {
        info!("✅ Bot {} is ready! Session: {}", ready.user.username, ready.session_id);
    }
    ...
    async fn on_at_message(&self, msg: &Message) {
        ...
        if msg.content.contains("你好") || msg.content.contains("hello") {
            if let Err(e) = self.api.reply_message(msg, "你好！我是 Rust QQ Bot！".to_string()).await {
    ...
    async fn on_c2c_message(&self, msg: &Message) {
        ...
        if let Err(e) = self.api.reply_message(msg, format!("Echo: {}", msg.content)).await {
            eprintln!("Failed to send reply: {}", e);
        }
    }
...
async fn main() -> Result<()> {
    ...
    let app_id = std::env::var("QQ_APP_ID")
        .expect("Missing QQ_APP_ID environment variable");
    let app_secret = std::env::var("QQ_APP_SECRET")
        .expect("Missing QQ_APP_SECRET environment variable");
    let use_sandbox = std::env::var("QQ_SANDBOX")
