...
- **Browser automation (basic)** — Optional `browser` feature with shared Chromium session controls, explicit browser executable env support, Playwright `chromium-*` / `chromium_headless_shell-*` cache discovery, CDP endpoint readiness probing, and raw CDP method dispatch: `browser_navigate`, `browser_snapshot`, `browser_click`, `browser_type`, `browser_scroll`, `browser_back`, `browser_press`, `browser_get_images`, `browser_console`, `browser_dialog`, `browser_screenshot`, `browser_vision`, and `browser_cdp`
...
- **Details**: Hakimi now covers navigate, snapshot, click, type, scroll, back, press, image listing, console/error capture, page-context expression evaluation, JavaScript dialog accept/dismiss, screenshot, screenshot-to-vision request routing, explicit executable env vars, Playwright cache/headless-shell discovery, and `browser_cdp` endpoint status/probe plus browser-level and target-level raw CDP method dispatch through the optional Rust-native Chromium feature. Remaining parity is multi-backend support (Browserbase/Browser Use/Camofox) and deeper CDP/supervisor frame routing state.
...
| 82a | Browser CDP Raw Dispatch | `hakimi-tools/src/builtin_browser.rs`, CLI/server/TUI registration | 10 | 🟡 Optional Chromium `browser_cdp` resolves explicit/env CDP endpoints, redacts signed URLs/userinfo before output, reports status without connecting, probes live browser version/target counts, and dispatches raw CDP methods at browser scope or after `Target.attachToTarget` for page targets; supervisor frame routing and cloud provider session routing remain future browser-backend work |
...
### Summary
- **Total tests**: 1674 (latest CI target; local compilation intentionally not run in automation)
