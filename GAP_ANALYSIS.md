...
| 99 | HTTP API Responses Endpoint | `hakimi-server/src/{api.rs,server.rs}` | 6 | ✅ `/v1/responses` accepts OpenAI-style string/message input, optional `instructions`, model override, in-memory `previous_response_id` chaining, GET/DELETE retrieval, and `stream=true` completed SSE snapshots without mutating shared `/api/chat` history |
...
### Summary
- **Total tests**: 1623 (latest CI target; local compilation intentionally not run in automation)
