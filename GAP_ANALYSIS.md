...
- **Browser automation (basic)** — Optional `browser` feature with shared Chromium session controls, explicit browser executable env support, Playwright `chromium-*` / `chromium_headless_shell-*` cache discovery, CDP endpoint readiness probing, provider CDP endpoint routing, cloud-provider readiness status, and raw CDP method dispatch: `browser_navigate`, `browser_snapshot`, `browser_click`, `browser_type`, `browser_scroll`, `browser_back`, `browser_press`, `browser_get_images`, `browser_console`, `browser_dialog`, `browser_screenshot`, `browser_vision`, and `browser_cdp`
...
- **Profile support** — `hakimi profile` manager with clone/export/distribution install flows, gateway `/profile`, sticky `active_profile`, and `--profile` CLI flag surface
...
- **Details**: Hakimi now covers navigate, snapshot, click, type, scroll, back, press, image listing, console/error capture, page-context expression evaluation, JavaScript dialog accept/dismiss, screenshot, screenshot-to-vision request routing, explicit executable env vars, Playwright cache/headless-shell discovery, and `browser_cdp` endpoint status/probe plus Browser Use, Browserbase, Firecrawl, Camofox, and local CDP readiness reporting. `browser_cdp` can also route to provider-specific CDP endpoint env vars using Hermes registry semantics: explicit provider wins, local disables cloud routing, auto selects Browser Use before Browserbase, and Firecrawl stays explicit-only. Remaining parity is real multi-backend session creation/routing and deeper CDP/supervisor frame routing state.
...
| 22 | Rate Limit Tracking + Gateway Usage + Cost Estimates | `hakimi-transports/src/rate_limit.rs`, `hakimi-common/src/usage_pricing.rs`, `hakimi-common/src/account_usage.rs`, transport adapters, `hakimi-cli/src/entry.rs` | 22 | ✅ OpenAI/Nous-style `x-ratelimit-*` parsing, detailed/compact formatting, hot-bucket warnings, latest snapshot retained by Chat/Responses/Anthropic/Gemini transports, and gateway `/usage` renders last-turn tokens/API calls, Hermes-style estimated cost, pricing snapshot version, rate-limit display, plus OpenRouter `/credits` and `/key` account quota/usage |
...
| 82a | Browser CDP Raw Dispatch | `hakimi-tools/src/builtin_browser.rs`, CLI/server/TUI registration | 21 | 🟡 Optional Chromium `browser_cdp` resolves explicit/env CDP endpoints, redacts signed URLs/userinfo before output, reports status without connecting, includes cloud-provider readiness for Browser Use, Browserbase, Firecrawl, Camofox, and local CDP without exposing secret values, probes live browser version/target counts, dispatches raw CDP methods at browser scope or after `Target.attachToTarget` for page targets, and routes provider-specific CDP endpoint env vars with Hermes-compatible explicit/auto provider semantics; supervisor frame routing and cloud provider session creation/routing remain future browser-backend work |
...
| 118 | OpenRouter Account Usage API | `hakimi-common/src/account_usage.rs`, `hakimi-cli/src/entry.rs` | 5 | ✅ Gateway `/usage` detects OpenRouter profiles, fetches `/credits` and `/key` with the configured API key, renders credits balance plus API-key quota/usage, and silently degrades to local token/cost/rate-limit output when account APIs are unavailable |

### Summary
- **Total tests**: 1718 (latest CI target; local compilation intentionally not run in automation)
