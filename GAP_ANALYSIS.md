...
- **Kanban tools (basic)** — `kanban_show`, `kanban_list`, `kanban_create`, `kanban_complete`, `kanban_block`, `kanban_unblock`, `kanban_comment`, `kanban_heartbeat`, `kanban_link`, `kanban_events`, `kanban_diagnostics`, `kanban_assign`, and `kanban_worker_log` persist tasks, comments, status transitions, liveness, dependency links, event trails, diagnostics, profile routing, worker logs, and isolated board routing in SQLite
...
- **Details**: Hakimi now covers 13 Hermes-aligned structured tool calls plus gateway `/kanban` CRUD/status/link/heartbeat/events/diagnostics/assign/worker-log operations, explicit board routing, profile/assignee routing, durable worker logs, and `/kanban boards list|show|create|switch` on isolated SQLite boards. Remaining parity is dispatcher-spawned workers, notification subscriptions, swarm creation, and dashboard-level management.
...
- **Status**: SQLite-backed tasks/comments/dependency links/events exist, and the agent tool surface exposes `kanban_show`, `kanban_list`, `kanban_create`, `kanban_complete`, `kanban_block`, `kanban_unblock`, `kanban_comment`, `kanban_heartbeat`, `kanban_link`, `kanban_events`, `kanban_diagnostics`, `kanban_assign`, and `kanban_worker_log`; gateway `/kanban` now performs real board operations, supports explicit `--board <slug>` routing, exposes event trails/diagnostics, routes tasks to profiles/assignees, persists durable worker logs, and provides `/kanban boards list|show|create|switch` for isolated board selection.
- **What's missing**: dispatcher-spawned workers, notification subscriptions, swarm creation, and dashboard-level management.
...
| 61 | Kanban Board Tool Surface | `hakimi-tools/src/builtin_kanban.rs`, `hakimi-cli/src/entry.rs`, server/TUI registration | 20 | ✅ SQLite-backed tasks, comments, status transitions, dependency links, heartbeats, event trails, diagnostics, profile routing, worker logs, 13 Hermes-aligned `kanban_*` tools, gateway `/kanban` CRUD/status/link/events/diagnostics/assign/worker-log operations, and isolated board routing |
